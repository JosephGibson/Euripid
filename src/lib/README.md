...
- `transactions.js` provides four helpers that record tagged Trends only (`withTransaction`, `withNavigation`, `withUserAction`, `withPageLoad`). They do not call k6 `group()` because browser scenarios are async and k6 forbids async `group()` callbacks. Typed helpers record only their own Trend so nested steps are not double-counted in `transaction_duration`. See `docs/RECIPES.md` for when to use each.
