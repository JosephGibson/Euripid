...
- **Groups (`root_group`)** — k6 may still render a collapsible **Groups** section for checks and any synchronous `group()` usage. Euripid’s transaction helpers **do not** use `group()` (k6 rejects async `group()` bodies; browser tests are async). **Per-step performance** comes from the custom transaction Trends below, keyed by the `transaction` tag on each sample.
...
For async browser scenarios, rely on **Trend rows** (`transaction_duration`, `navigation_duration`, `user_action_duration`, `page_load_duration`) and their **`transaction` tags** for step-level timing. The Groups tree is not driven by `withTransaction` / `withNavigation` / `withUserAction` / `withPageLoad`; those helpers only record tagged Trends.
...
Every custom Trend, Counter, and Rate is under `metrics`. k6’s group/check tree is under `root_group`. With Euripid’s async transaction helpers, `root_group.groups` is often sparse; step names still appear on Trend samples via the `transaction` tag in the full summary when using `EURIPID_WRITE_FULL_SUMMARY=true` or the raw `k6-stream.json` output.
